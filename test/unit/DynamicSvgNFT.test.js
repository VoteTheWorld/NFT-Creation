...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("DynamicSvgNFT test", function () {
          let DynamicNFT, deployer, mockV3Aggregator
          beforeEach(async () => {
              accounts = await ethers.getSigners()
              deployer = accounts[0]

              await deployments.fixture(["mock", "DynamicSvgNFT"])

              DynamicNFT = await ethers.getContract("DynamicSvgNFT")
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator")
          })

          describe("constructor initiated", function () {
              it("constructor", async () => {
                  ...
                  const lowImageURI = await DynamicNFT.getLowSvgImageURI()
                  const highImageURI = await DynamicNFT.getHighSvgImageURI()

                  assert.equal(name, "Dynamic SVG NFT")
                  assert.equal(symbol, "SVG")
